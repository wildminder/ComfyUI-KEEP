...
try:
    ...
    # Minimal fallback if wm_basicsr isn't in path when facelib is directly used (should not happen with our setup)
...
def init_detection_model(model_name, half=False, device='cuda', model_rootpath=None):
    ...
    if not os.path.exists(model_path_final):
        ...
        # This load_file_from_url is from wm_basicsr, which should be in sys.path
